 ...
 * PARSEO
 ***********************/
function extractLiters(html) {
  if (!html) return null;

  const clean = html.replace(/\s+/g, " ");
  const re = new RegExp(
    "EQUIPETROL[\\s\\S]*?Volumen disponible[\\s\\S]*?(\\d{1,3}(?:,\\d{3})*)\\s*Lts",
    "i"
  );
...
const litros = extractLiters(html) ?? 0;
const now = new Date();
...
// Menos padding arriba para subir el título
w.setPadding(8, 16, 16, 16);

// ── TÍTULO
const title = w.addText("Equipetrol");
...
// Espacio mínimo debajo del título
w.addSpacer(4);
...
// ⬇️ AÚN MÁS ESPACIO ANTES DE LA LÍNEA DE CONSULTA
w.addSpacer(28);
